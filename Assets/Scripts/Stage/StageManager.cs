...
public class StageManager : MonoBehaviour
{
    ...
    public void TempFuncTextByType(string notetype)
    {
        if (currentComboCoroutine != null)
        {
            StopCoroutine(currentComboCoroutine);
    ...
    {
        RectTransform textTransform = comboText.GetComponent<RectTransform>();
        Vector3 originalPosition = textTransform.localPosition; // 원래 위치 저장
    ...
    public void RestartGame()
    {
        if (GameManager.Instance != null)
        {
            ...
            GameManager.Instance.TransitionToScene(currentScene);
        }
        isGameOver = false;
    }
